...
class Conn {
	...
	private function __config() {
		...
		if(!file_exists($path)) {
			...
			if($_SERVER['HTTP_HOST'] == 'localhost') {
				...
				$this->_driver = $config['live_driver'];
				$this->_host = $config['live_host'];
				$this->_user = $config['live_user'];
				$this->_pass = $config['live_pass'];
				$this->_db = $config['live_db'];
			}
		}
	}
	...
	*/
	private function _conn() {
		try {
			$this->__config();
			$this->conn = new PDO($this->_driver . ':host=' . $this->_host . ';dbname=' . $this->_db . ';charset=utf8', $this->_user, $this->_pass);
